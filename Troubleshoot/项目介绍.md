...
- 实验环境基于本地 VMware Workstation，虚拟机包含 `master01`、`node01`、`node02`，其中 `master01` 同时作为本组 NFS 服务器。可以根据自己机器实际配置调整虚拟机配置。
- 由于 IP 冲突与组网调整，需要将集群关键组件中的 `10.3.201.*` 统一变更为 `10.3.204.X`，引发控制平面连接异常、证书 SAN 不匹配、kube-proxy 配置错误、Calico token 创建失败等问题。
...
- 环境与系统：`VMware Workstation`、`Rocky Linux`
- 容器与编排：`Docker`、`cri-dockerd`、`Kubernetes (kubeadm)`
- 网络与入口：`Calico`、`CoreDNS`、`kube-proxy`、`Ingress`
- 存储与配置：`NFS`、`PV/PVC`、`StorageClass`、`ConfigMap`、`Secret`
- 业务与数据：`LNMP`、`Redis`
- 监控与扩缩容：`Metrics Server`、`HPA`、`VPA`
- 高可用：`HAProxy`、`Keepalived`、etcd集群、MySQL 主从、Redis Cluster
- 排障工具：`OpenSSL`、`journalctl`

## 实战要求

1. 每天中午12点前发布任务和视频
2. 需要在第二天中午12点前完成任务并输出故障排查和恢复过程文档记录/恢复后截图
3. 扩展部分不做时间要求，根据自身情况完成选择性完成即可
...
1. 配置好本地实验环境
2. 将每个节点的IP地址更改为自己所属分组的网段IP
3. `kubectl get nodes -o wide`命令输出的集群信息异常，排查并恢复正常
